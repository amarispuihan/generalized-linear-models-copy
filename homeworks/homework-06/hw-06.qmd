...
```{r}
nature$Treatment <- relevel(nature$Treatment, ref = "URBAN (T1)")
model_probit <- glm(
  donation_binary ~ Age + Gender + Treatment + nep_high + nep_high:Treatment,
  data = nature,
...
The coefficient on Treatment (Nature T2) is positive, suggesting that watching the documentary increases the likelihood of donating relative to the Urban group. However, this effect is not statistically significant (p = 0.637), indicating that there is no strong evidence that the documentary impacts donation behavior. Additionally, the interaction with environmental attitudes (nep_high) is not significant, suggesting the effect does not differ meaningfully across individuals.

```{r}
install.packages("margins")  
library(margins)
margins(model_probit, variables = "Treatment")
```

The marginal effect of Treatment (Nature T2) indicates that watching the documentary increases the probability of donating by approximately 2.45 percentage points relative to the Urban group, holding other factors constant. However, this effect is small and, based on earlier results, not statistically significant.
...
A 20-year-old female in the Nature (T2) group with low NEP has about a 93.0% predicted probability of donating.
